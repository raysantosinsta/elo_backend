  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleConnection(client: Socket) {
    ...
    // 🔥 VALIDAÇÃO: Observador SEMPRE precisa de driverId
    if (!isDriver && !driverId) {
      this.logger.warn(
        `Observador ${client.id} conectou sem driverId! Desconectando...`,
      );
      client.disconnect();
      return;
    }
    ...
    if (driverId && routeId) {
      ...
      if (isDriver) {
        ...
        this.logger.log(
          `👀 OBSERVADOR ${client.id} monitorando motorista ${driverId} na rota ${routeId}`,
        );
        ...
        if (lastLoc) {
          ...
          this.logger.log(
            `📡 Enviando última localização para observador ${client.id}`,
          );
        } else {
          this.logger.log(
            `⏳ Nenhuma localização anterior para motorista ${driverId}`,
          );
        }
      }
    }
  ...
  handleUpdateLocation(
    ...
    if (!driverId || !routeId) {
      this.logger.warn(
        `Cliente ${client.id} tentou atualizar localização sem driverId/routeId`,
      );
      return;
    }
    ...
    if (now - lastUpdateTime < this.THROTTLE_MS) {
      ...
      this.logger.debug(
        `⏳ Throttle ativo para motorista ${driverId} (${now - lastUpdateTime}ms desde última atualização)`,
      );
      return;
    }
    ...
    if (!throttleInfo) {
      ...
      if (throttleInfo.updateCount > this.MAX_UPDATES_PER_MINUTE) {
        this.logger.warn(
          `⚠️ Motorista ${driverId} excedeu limite de atualizações (${throttleInfo.updateCount}/${this.MAX_UPDATES_PER_MINUTE})`,
        );
        return;
      }
    }
    ...
    if (lastLocation) {
      ...
      if (distance < 5 && lastLocation.isSimulating === data.isSimulating) {
        this.logger.debug(
          `📍 Motorista ${driverId} movimentou apenas ${distance.toFixed(1)}m, ignorando...`,
        );
        return;
      }

      this.logger.debug(
        `📍 Motorista ${driverId} movimentou ${distance.toFixed(1)}m`,
      );
    }
  ...
  handleRouteFinished(
    ...
    this.logger.log(
      `🏁 Rota finalizada: ${routeId} pelo motorista ${driverId}`,
    );
